...
class RuleKnowledgeExtractor:
    ...
    def _extract_law(self, text: str, max_children: int, max_depth: int) -> List[Dict[str, Any]]:
        ...
        for ch_start, ch_end, ch_num, ch_name in chapter_ranges:
            # 收集该章内的条
            ch_articles = [
                (a_start, a_end, a_num) for a_start, a_end, a_num in article_positions if ch_start <= a_start < ch_end
            ]

            ch_node = {"name": ch_name, "description": f"第{ch_num}章，共{len(ch_articles)}条", "children": []}
                ...
                for group in groups:
                    if len(groups) == 1:
                        ...
                        for a_start, a_end, a_num in group:
                            ...
                            ch_node["children"].append({"name": f"第{a_num}条", "description": summary, "children": []})
                    else:
                        # 多组，每组作为一个二级节点
                        first_num = group[0][2]
                        last_num = group[-1][2]
                        ...
                        group_node = {"name": f"第{first_num}-{last_num}条", "description": group_desc, "children": []}

                        for a_start, a_end, a_num in group:
                            summary = self._get_first_sentence(text, a_end)
                            group_node["children"].append(
                                {"name": f"第{a_num}条", "description": summary, "children": []}
                            )
    ...
    def _group_articles(self, text: str, article_positions: list, max_children: int) -> List[Dict[str, Any]]:
        ...
        for group in groups:
            ...
            for a_start, a_end, a_num in group:
                summary = self._get_first_sentence(text, a_end)
                group_node["children"].append({"name": f"第{a_num}条", "description": summary, "children": []})
    ...
    def _extract_markdown(self, text: str, max_children: int, max_depth: int) -> List[Dict[str, Any]]:
        ...
        for i, h1 in enumerate(h1_list):
            ...
            node = {"name": h1.group(1).strip(), "description": "", "children": []}
            ...
            if h2_list:
                for j, h2 in enumerate(h2_list):
                    ...
                    h2_node = {"name": h2.group(1).strip(), "description": self._get_summary(h2_text), "children": []}

                    # 找 h3
                    if max_depth >= 3:
                        h3_list = list(self.PATTERNS["h3"].finditer(h2_text))
                        for h3 in h3_list[:max_children]:
                            h3_node = {"name": h3.group(1).strip(), "description": "", "children": []}
    ...
    def _extract_numbered(self, text: str, max_children: int, max_depth: int) -> List[Dict[str, Any]]:
        ...
        for i, h1 in enumerate(h1_list):
            ...
            node = {"name": h1.group(2).strip(), "description": "", "children": []}
            ...
            if h2_in_range:
                for j, h2 in enumerate(h2_in_range[:max_children]):
                    ...
                    h2_node = {"name": h2.group(2).strip(), "description": "", "children": []}

                    # 找 h3
                    if max_depth >= 3:
                        h3_in_range = [m for m in h3_list if h2_end <= m.start() < next_h2]
                        for h3 in h3_in_range[:max_children]:
                            h3_node = {"name": h3.group(2).strip(), "description": "", "children": []}
                            h2_node["children"].append(h3_node)
    ...
    def _extract_plain(self, text: str, max_children: int, max_depth: int) -> List[Dict[str, Any]]:
        ...
        for i in range(0, min(len(paragraphs), 50), max_children):
            ...
            for j, para in enumerate(group):
                ...
                group_node["children"].append({"name": first_sentence[:30], "description": para[:150], "children": []})
            tree.append(group_node)
...
def extract_knowledge_by_rules(text: str, max_children: int = 5, max_depth: int = 3) -> Dict[str, Any]:
    """基于规则提取知识点
    ...
    # 后处理：为每个节点补充 excerpt 字段（原文内容）
    # rule extractor 的 description 是简短摘要，excerpt 保留完整原文段落
    def ensure_excerpt(nodes):
        for n in nodes:
            if "excerpt" not in n or not n.get("excerpt"):
                n["excerpt"] = n.get("description", "") or ""
            if n.get("children"):
                ensure_excerpt(n["children"])

    ensure_excerpt(knowledge_points)
